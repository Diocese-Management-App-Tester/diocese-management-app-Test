...
// ---------- دعوة مخدوم (QR) — scoped link to /child/signup (migration 0042) ----------
...
export default function ChildInvitePanel() {
  ...
  const inviteUrl = useMemo(() => {
    ...
    const url = new URL('/child/signup', window.location.origin);
  ...
  const shareLink = async () => {
    try {
      await navigator.share({ title: 'دعوة للانضمام كمخدوم', url: inviteUrl });
    } catch {
    ...
    <>
      <section id="invite-scope" className="card space-y-3 mb-4">
        <p className="text-sm font-bold text-gray-600">
          نطاق الدعوة — سيتم تحديد هذه الاختيارات مسبقاً للمخدوم عند التسجيل:
        </p>
      ...
      <p className="px-2 text-center text-xs text-gray-400 leading-relaxed">
        المخدوم الذي يفتح هذا الرابط سيجد الفصل المحدد مقفولاً في الخطوة الأولى، ثم يُدخل كوده
        (كتابةً أو بمسح الـ QR) وبياناته وكلمة المرور — ويظهر طلبه في «الطلبات» ليعتمده خادم الفصل.
      </p>
    </>
  );
}
