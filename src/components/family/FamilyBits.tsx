...
import {
  ...
  removeFamilyMember, setFamilyMemberRelation,
  type Family, type FamilyMemberWithPerson, type FamilyRelation,
} from '@/lib/families';
    ...
    <li id={`family-${family.id}`} className="card !py-3 !border-2 !border-teal-100">
      <div className="flex items-center gap-3">
        ...
        <div className="flex shrink-0 items-center gap-1">
          ...
          {canManage && (
            <>
              <button type="button" onClick={onAddMembers} aria-label="إضافة أفراد بالمسح" title="إضافة أفراد (QR)"
...
export function FamilyFormModal({
  ...
  const submit = async () => {
    ...
    const payload = { name: n, phone: phone.trim() || null, address: address.trim() || null, notes: notes.trim() || null };
    const q = family
    ...
    setBusy(false);
    if (err || !data) {
      setError(err?.message?.includes('row-level security') ? 'ليس لديك صلاحية إدارة العائلات' : 'تعذر الحفظ، حاول مجدداً (تأكد من تطبيق تحديث قاعدة البيانات)');
      return;
    }
  ...
  return (
    <ModalFrame title={family ? 'تعديل العائلة' : 'عائلة جديدة'} icon={<UsersRound className="h-5 w-5 text-teal-600" />} onClose={onClose}>
      ...
      <input id="family-name" className="input-field mb-3" value={name} onChange={(e) => setName(e.target.value)} placeholder="مثال: عائلة جرجس" autoFocus />
      ...
      <button id="family-save" type="button" onClick={submit} disabled={busy} className="btn-primary flex w-full items-center justify-center gap-2">
