...
2. SQL Editor → **fastest path:** run `supabase/full_schema.sql` once — it is every migration (`0001` → `0040`, except `0002`) combined into one file, verified to produce a schema identical to running the files one by one. Regenerate it after adding a migration with `supabase/build_full_schema.sh`.
   Alternatively run **all** migrations in `supabase/migrations/` in numeric order (`0001` → `0040`). Either way `0002_bootstrap_owner.sql` runs after step 5.
