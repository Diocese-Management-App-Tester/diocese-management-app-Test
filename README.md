...
**إدارة المخدومين → المخدومين** (`/children/manage`, default tab, `?tab=people`) — `src/components/children/ManagePeoplePanel.tsx` — the ONE place to **edit · stop · delete** people:
- **المخدومين | الخدام** switch: children (enrollments `kind = 'child'`) and servants (their mirror enrollments — actions go through the servant ACCOUNT: `EditServantModal`, `servant_enrollments.status`, delete; scoped by `canManageServant`).
- Scope selectors (كنيسة → خدمة → فصل), search, **status filter** (الكل / يعمل / موقوف) and **«ترتيب حسب» الكنيسة / الخدمة / الفصل** (`src/components/ScopeOrganizer.tsx`, shared with **إدارة الخدام → الخدام**, which got the same organizer). Every group has a sticky header with **إيقاف الكل / تفعيل الكل** for that scope.
- Per person: **تعديل** (`EditPersonModal` incl. code + password reset) · **إيقاف / تفعيل** · **حذف** (`DeletePersonModal`: this enrollment or the whole person).
- **إيقاف / تفعيل نطاق كامل** card: pick a **church**, a **service** or a **class** and who is affected (المخدومين / الخدام / الجميع) → RPC **`set_enrollments_status(status, ids | church → service → class, kind)`** (SECURITY DEFINER, `can_access` on the scope; servants are suspended / re-approved through `can_manage_servant`).
...
- ✅ إدارة الخدام: edit / suspend / delete scoped per level (`/servants`), servant photos, **ترتيب حسب الكنيسة / الخدمة / الفصل (0043)**
- ✅ **إدارة المخدومين → المخدومين (0043)**: edit · stop · delete children AND servants, organized by church / service / class, stop a whole class / service / church; children page & scanner are view-only for data; default password `000000`; servant password change = old + new + confirm, superiors reset without the old one
